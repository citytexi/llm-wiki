...
위키가 커지면 `[[위키링크]]`만으로는 안 보이는 관계가 생긴다. "링크는 걸려 있는데
개념적으로는 고립된 페이지", "여러 주제를 잇는 다리 역할을 하는 페이지" 같은 것들이다.
`wiki/script/graph_signals.py`가 그걸 lint 경고로 낸다.

이 모듈은 **그래프 생성 도구를 import하지 않는다.** `graphify-out/graph.json`이라는
산출물 파일만 읽는다. 그래서 어떤 도구로 만들든 형식만 맞으면 동작하고, 아무것도 설치하지
않았으면 신호를 하나도 내지 않는다. 그래프를 만든 적 없는 저장소는 정상 상태다.

이 골격을 뽑아낸 저장소에서는 `graphify`라는 외부 CLI 도구로 만들었다. 그 도구는 이
저장소에 들어 있지 않다 — 제3자 소프트웨어라 재배포하지 않는다. 직접 구하거나, 같은
스키마의 `graph.json`을 내는 다른 도구를 쓴다.

붙이면 켜지는 신호 4종(전부 경고, 위반 아님):

| 코드 | 뜻 |
|---|---|
| `그래프stale` | 그래프가 현재 `wiki/domains/` 내용과 어긋난다. 이때 나머지 셋은 판정을 생략한다 |
| `그래프고립` | 개념 그래프에서 차수 ≤ 1인 페이지 |
| `그래프희소` | 내부 밀도가 낮은 커뮤니티 |
| `그래프브리지` | 여러 커뮤니티를 잇는 페이지 |

전부 "고칠 것"이 아니라 "볼 것"이다. 위반으로 만들면 그래프가 재생성될 때마다 커밋이
막힌다. 자세한 판정 기준은 `wiki/references/lint-rules.md`에 있다.

전제 두 가지:

- 스캔 루트는 `wiki/domains/`다. `wiki/script/`의 파이썬은 그래프에 넣지 않는다.
  `graph_signals.SCAN_ROOT`와 `graphify-out/.graphify_root`가 어긋나면 `그래프stale`이
  뜬다
- 그래프를 붙였다면 `wiki/routing.json`의 `research.reference`에
  `graphify-out/GRAPH_REPORT.md`를 더해도 된다. **붙이기 전에는 넣지 않는다** —
  `route.py`가 routing.json이 가리키는 경로의 실재를 검증하므로, 없는 파일을 적으면 모든
  의도에서 종료 코드 4로 막힌다
